...
"""
############ IMPORT MODULES ############
import os, datetime, subprocess, csv, os.path, glob
...
p_equatorial = np.zeros([number_of_ypoints,number_of_xpoints])
cs_eq = ax1.scatter(yyy,xxx,c=p_equatorial,s=msize,edgecolors='none',norm=mpl.colors.LogNorm(vmin=vmin, vmax=vmax),cmap='jet')
cb = fig.colorbar(cs_eq,orientation='horizontal',cax=cb_ax)
...
p_meridian = np.zeros([number_of_xpoints,number_of_zpoints])  
cs_mer = ax2.scatter(xxxx,zzzz,c=p_meridian,s=msize,edgecolors='none',norm=mpl.colors.LogNorm(vmin=vmin, vmax=vmax),cmap='jet')
