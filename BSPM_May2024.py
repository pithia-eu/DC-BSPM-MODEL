...
20240123
- change to get noaa and gfz kp from Iswa

20231213
- bug found; map and zip re-introduced
- map and zip introduced recently not working (check why?). They were replaced by the old loops. 

20231123
- ed_input and fkp_main fortran routines replaced by python translations
- vertical line on kp plot added
...
from ed_inputE import ed_input
from fkp_main import fkp_main
import get_kp 
#import png2gif

# Ininialize parameters
parser = argparse.ArgumentParser(description='BSPM')
...
parser.add_argument("--troughParam", default='VAP', type=str) # other: 'Sheeley', 'VAP', 'CA'
parser.add_argument("--executionid", default=datetime.datetime.now().strftime('%Y-%m-%d'), type=str) 
parser.add_argument("--kpdata", default='NOAA', type=str) # other: 'GFZ'
...
cmap = copy.copy(mpl.cm.get_cmap("turbo")) ; cmap.set_bad(color='white')
...
#        url='https://chain-new.chain-project.net/echaim_downloads/'
        url='https://irimodel.org/indices/'
...
# def mer_coord_grid(MLT):
#     global MLT1, MLT2

#     # Calculate MLT1 and MLT2
#     if 6.0 <= MLT < 18.0:
#         MLT1, MLT2 = MLT, (MLT + 12) % 24
#     else:
#         MLT1, MLT2 = (MLT - 12), MLT if MLT >= 18 else (MLT + 12), MLT

#     # Generate the grid
#     x_range = np.linspace(-xmax, xmax, number_of_xpoints)
#     z_range = np.linspace(-zmax, zmax, number_of_zpoints)
#     x_array_solar_magnetic_mer, z_array_solar_magnetic = np.meshgrid(x_range, z_range)

#     # Calculate radius_in_R_E_mer
#     radius_in_R_E_mer = np.hypot(x_array_solar_magnetic_mer, z_array_solar_magnetic)

#     # Calculate latitude_in_degrees_mer using vectorized operations
#     non_zero_x_coord = x_array_solar_magnetic_mer != 0
#     latitude_in_degrees_mer = np.rad2deg(np.arctan2(z_array_solar_magnetic, np.abs(x_array_solar_magnetic_mer)))
#     latitude_in_degrees_mer[~non_zero_x_coord] = 0

#     # Calculate magnetic_local_time_mer using vectorized operations
#     magnetic_local_time_mer = np.where(x_array_solar_magnetic_mer >= 0, MLT1, MLT2)

#     return x_array_solar_magnetic_mer, z_array_solar_magnetic, radius_in_R_E_mer, magnetic_local_time_mer, latitude_in_degrees_mer
...
#####################################################################################
def compute_p_equatorial(k, k_slideE):
    return get_plasma_ionosphere_density_electrons_19jan2016(
        r=radius_in_R_E_eq[k], lambdaE=lambda_equatorial,
        MLT=magnetic_local_time_eq[k], year=year, month=month, day=day,
        k_slide=k_slideE, nspot=nspot,
        iri_max_height=iri_max_height, add_trough=add_trough,
        a_plasmapause=plasmapause[k_slideE][1],
        a_clean=plasmapause[k_slideE][0], troughParam=troughParam
    )

#####################################################################################
def compute_p_meridian(k, k_slideE):
    return get_plasma_ionosphere_density_electrons_19jan2016(
        r=radius_in_R_E_mer[k], lambdaE=latitude_in_degrees_mer[k],
...
def animate (radius_in_R_E_eq,magnetic_local_time_eq,radius_in_R_E_mer,magnetic_local_time_mer,latitude_in_degrees_mer,var,xkp,ykp,plasmapause,dfk,MLT1):
    ...
    for k_slide in range(ini,nt,step):
        ...
        print('STARTING HOUR: ', n, '/', int((nt-ini+1)/2), ' (', str(integer_hour).zfill(2),'h',str(integer_minutes).zfill(2),'m)')
        
        kp=np.float64(dfk.loc[str(year)+'-'+str(month).zfill(2)+'-'+str(day).zfill(2)+' '+str(integer_hour).zfill(2)+':'+str(integer_minutes).zfill(2)+':00'])
...
#####   EQUATORIAL PLANE
        ...
        # Compute p_equatorial values for each k in new_eq
        p_equatorial_indices = map(compute_p_equatorial, new_eq, len(new_eq)*[k_slide])        
        # Update p_equatorial using the computed values
        for k, value in zip(list(new_eq), list(p_equatorial_indices)): p_equatorial[k] = value
        
        #print(p_equatorial)
        
      
#        L_eq = radius_in_R_E_eq / np.cos(np.deg2rad(lambda_equatorial))**2
...
#        L_eq = L_eq.reshape(number_of_xpoints,number_of_ypoints)
...
#####   MERIDIAN PLANE    
        ...
        # Compute p_meridian values for each k in new_mer
        p_meridian_indices = map(compute_p_meridian, new_mer, len(new_mer)*[k_slide])        
        # Update p_equatorial using the computed values
        for k, value in zip(list(new_mer), list(p_meridian_indices)): p_meridian[k] = value
    
        L_mer = radius_in_R_E_mer / np.cos(np.deg2rad(latitude_in_degrees_mer))**2
                       
        filename_results_mer='dens_mer_' + time_plot + timestamp + '.csv'
        df=pd.DataFrame([radius_in_R_E_mer,magnetic_local_time_mer,latitude_in_degrees_mer,L_mer,p_meridian]).transpose()
        df.columns=['r[Re]','MLT[hours]','Lat[Deg]','L','density_mer[1/cm\u00b3]']
#### for Khazanov
        df=df.loc[(df['density_mer[1/cm\u00b3]']>0.0) & (df['r[Re]']<1.16)]
        df=df.loc[(df['L']>=5.3) & (df['L']<=5.5)]

        df.to_csv(user_path_out+filename_results_mer, sep=',',float_format='%.3f',index=False)

#        L_mer = L_mer.reshape(number_of_xpoints,number_of_zpoints)
...
#         L = (L_eq >= 5.39) & (L_eq <= 5.41)
#         ax1.scatter(-yyy[L],-xxx[L],marker='x',c='k')       
#         L = (L_mer >= 5.39) & (L_mer <= 5.41)
#         ax2.scatter(-xxxx[L],zzzz[L],marker='x',c='k')
...
# Temporary to check PP        
        ...
        dfp.to_csv(user_path_out+filename_PP, sep=',',float_format='%.6f',index=False)
# Temporary to check PP                      

        ax4.text(0.1, 0.5,time_plot,fontsize=26,fontweight="bold")
#        ax4.text(0.1, 0.5,'March 1 2013 02H',fontsize=26,fontweight="bold")
        ax5.text(0.2, 0.5, title ,fontsize=22,fontweight="bold")
        ax6.set_title('Axes units in Re',size=16)        
        ax7.set_title('Belgian SWIFF Plasmasphere Model v.2021',size=18, fontstyle='italic')
...
#        ax3.bar(xkp,ykpcut,bar,color='k',align='edge')
        ...
        ax3.set_xlim(0,72)
        ax3.axvline(x = integer_hour+24, color = 'r', ls='--', linewidth=4)
# End Print Kp bars
...
def fig_init (xkp,MLT1):
    ...
    ax1.set_title('Equatorial plane',fontsize=22,fontweight='bold')
    for c in circles:
    ...
    if MLT1==12: ax2.set_title('Meridian plane',fontsize=22,fontweight='bold')
    else: ax2.set_title('Meridian plane (MLT='+str(MLT1)+')',fontsize=22,fontweight='bold')
    for c in circles:
...
#    ax3.set_title('Feb 28 2013'+'                          '+'Mar 1 2013'+'                          '+'Mar 2 2013')
    ax3.set_ylabel('Kp',fontsize=14)
#    
    ...
    ax7 = fig.add_subplot(gs[4:5,-4:]); ax7.axis('off')
...
get_kp.Kp(kpdata, year, month, day, user_path_in)
